<script lang="ts">
    ...
    async function handleToggle(e: Event) {
        ...
        // Optimistic update
        const previousPower = light.power;
    ...
    <div class="flex justify-between items-start relative z-10">
        <div class="flex items-center gap-4">
            ...
            <div class="relative w-12 h-12 shrink-0">
                <div
                    ...
                    style="
                        ...
                        opacity: {light.power === 'on' ? 1 : 0.2};
                        box-shadow: 0 0 {light.power === 'on'
                        ? 20
                        : 0}px {previewColor};
                        transform: scale({light.power === 'on' ? 1 : 0.9});
                    "
                ></div>
                <!-- Online Status Dot -->
                <div
                    class="absolute -bottom-1 -right-1 w-3 h-3 bg-surface rounded-full flex items-center justify-center"
                >
                    {#if !light.connected}
                        <div
                    ...
                    {:else}
                        <div
                            ...
                            class:bg-green-500={light.power === "on"}
                            class:bg-neutral-500={light.power === "off"}
                        ></div>
                    {/if}
                </div>
            </div>
            ...
            <div class="flex flex-col" class:opacity-50={!light.connected}>
                <h3
                    class="font-semibold text-white text-lg leading-tight tracking-tight"
                >
                    {light.label}
                </h3>
                <p
                    class="text-xs font-medium text-text-tertiary uppercase tracking-wider mt-1"
                >
                    {#if !light.connected}
        ...
        <button
            ...
            class="w-12 h-7 rounded-full relative transition-all duration-300 ease-out focus:outline-none focus:ring-2 focus:ring-glass-active {light.power ===
            'on'
                ? 'bg-white'
                : 'bg-glass-hover'} disabled:opacity-50 disabled:cursor-not-allowed"
        >
            <span
                class="absolute left-1 top-1 w-5 h-5 rounded-full transition-all duration-300 ease-out shadow-sm {light.power ===
                'on'
